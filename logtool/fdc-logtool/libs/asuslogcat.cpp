...
//static bool checkNameValidate(char* path);
...
static void show_fail(const char *cmd){
	  fprintf(stderr,"fail reseason : %s\n", cmd);
}
...
//        logprint_run_tests();
    ...
    for (;;) {
        ...
        ret = getopt(argc, argv, "cdu:t:gsQf:r::n:v:b:B");
        ...
        switch(ret) {
            ...
            case 'u': {
                char *buf = (char*) malloc(128 + strlen(optarg) + 1);
		property_get("ro.uts.logging.path" , buf, "/data/uts/");
		strcat(buf, optarg);
                android::g_outputFileName = buf;
		}
            break;
            ...
            case 'f':
            	...
            	if(android::checkNameValidate(optarg)==false){
            		android::show_fail("path invalidate!!!");
            		exit(-1);
            	}
            	//temp
